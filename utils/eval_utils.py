...
def plot_traj_colormap(ax, traj, array, plot_mode, min_map, max_map):
    pos = traj.positions_xyz
    if plot_mode == PlotMode.xy:
        points = pos[:, [0, 1]]
        ...
        raise ValueError(f"Unsupported plot mode for 2D colormap: {plot_mode}")

    if len(points) < 2:
        return

    segments = np.stack([points[:-1], points[1:]], axis=1)
    norm = colors.Normalize(vmin=min_map, vmax=max_map, clip=True)
    ...
    line_collection.set_array(np.asarray(array[:-1]))
...
def eval_rendering(
    ...
    viz_save_dir = os.path.join(save_dir, "viz", str(iteration))
    pred_save_dir = os.path.join(viz_save_dir, "pred")
    gt_save_dir = os.path.join(viz_save_dir, "gt")
    compare_save_dir = os.path.join(viz_save_dir, "compare")
    mkdir_p(pred_save_dir)
    mkdir_p(gt_save_dir)
    mkdir_p(compare_save_dir)
    ...
    for idx in range(0, end_idx, interval):
        ...
        cv2.imwrite(os.path.join(pred_save_dir, filename), pred)
        cv2.imwrite(os.path.join(gt_save_dir, filename), gt)
        cv2.imwrite(
            os.path.join(compare_save_dir, filename),
